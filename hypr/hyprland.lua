...
hl.config({
	general = {
		...
		col = {
			active_border = {
				colors = { "rgba(cba6f7aa)", "rgba(11111bee)" },
				angle = 45,
			},
...
local function cycle_layout()
	...
	-- Show notification
	hl.exec_cmd("notify-send -u low -t 1000 'Current Layout' '" .. current_layout .. "'")
end

-- local function sync_master_to_active_window()
...
-- hl.on("window.active", sync_master_to_active_window)

-- Example binds, see https:y//wiki.hypr.land/Configuring/Basics/Binds/ for more
...
hl.bind(mainMod .. " + D", hl.dsp.exec_cmd(discordClient))
...
-- Screenshot
hl.bind("PRINT", hl.dsp.exec_cmd(screenshotTool .. " -z --clipboard-only -m output"))
hl.bind("SHIFT + PRINT", hl.dsp.exec_cmd(screenshotTool .. " -z --clipboard-only -m region"))
...
hl.bind("XF86Launch6", hl.dsp.exec_cmd("wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle"), {
	locked = true,
	repeating = true,
})
